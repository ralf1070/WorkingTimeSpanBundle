# Working Time Span Bundle

A Kimai plugin that provides an alternative working time calculation based on time spans instead of summing individual entries.

## The Problem

Kimai's default working time calculation sums up the duration of all time entries. This causes issues when entries overlap:
...
The default calculation counts overlapping time twice. This plugin calculates the actual time span from first start to last end.
...
|---------|---------|-------------|
| Enable alternative calculation | Off | Enable/disable the plugin |
