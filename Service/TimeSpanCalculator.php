...
namespace KimaiPlugin\WorkingTimeSpanBundle\Service;
...
final class TimeSpanCalculator
{
    ...
    private function filterByMaxDuration(array $timesheets, int $maxDuration): array
    {
        return array_filter($timesheets, function (Timesheet $entry) use ($maxDuration) {
            $duration = $entry->getEnd()->getTimestamp() - $entry->getBegin()->getTimestamp();
                ...
                if ($dayKey === $expectedNextDay) {
                    ...
                    if ($previousLatestEnd > $midnight) {
                        ...
                        foreach ($groupedByDay[$dayKey] as $entry) {
                            $gap = $entry->getBegin()->getTimestamp()
                                    - $previousLatestEnd->getTimestamp();

                            if ($gap <= $gapTolerance) {
                                $entriesToMove[] = $entry;
                                // Update latestEnd if this entry extends further
                                if ($entry->getEnd() > $previousLatestEnd) {
                                    $previousLatestEnd = $entry->getEnd();
                                }
                            } else {
                                $entriesToKeep[] = $entry;
                            }
     ...
     *
     * @param array $entries Timesheet objects or virtual entries
     * @return array<array{start: \DateTimeInterface, end: \DateTimeInterface, entries: array}>
     */
    private function mergeEntriesToSpans(array $entries, int $gapTolerance): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            $gap = $entryStart->getTimestamp() - $currentSpan['end']->getTimestamp();

            if ($gap <= $gapTolerance) {
                // Merge: extend span if needed
                if ($entryEnd > $currentSpan['end']) {
     ...
     * @param array &$carryOver Reference to carry-over array for next days
     * @return array Processed spans for current day
     */
    ...
    private function calculateSpansDuration(array $spans): int
    {
        ...
        return max(0, $totalDuration - $totalBreaks);
    }
}
